...
object ChessMoveDetector {
    ...
    private fun detectPiecesOnBoard(
        ...
        for (r in 0 until 8) {
            for (c in 0 until 8) {
                ...
                val localBg = if (patchMeans.isNotEmpty()) {
                    val sorted = patchMeans.sorted()
                    if (sorted.size % 2 == 0) {
                ...
                val localBgStd = if (patchStds.isNotEmpty()) {
                    val sorted = patchStds.sorted()
                    if (sorted.size % 2 == 0) {
                        (sorted[sorted.size / 2 - 1] + sorted[sorted.size / 2]) / 2.0
                    } else {
                        sorted[sorted.size / 2]
    ...
    fun getBoardState(imgPath: String, boardName: String): Map<String, Set<String>>? {
        ...
        if (img.empty()) {
            Log.e("ChessDetector", "Ã¢ÂÅ’ Could not load $imgPath")
            return null
        }

        val (detectedImg, innerPts) = detectLargestSquareLike(img)
        if (innerPts == null) {
            Log.e("ChessDetector", "Ã¢ÂÅ’ No board found")
            return null
        }
        ...
        Log.d("ChessDetector", "Ã¢Å“â€¦ $boardName: ${whiteSquares.size} white, ${blackSquares.size} black pieces")
    ...
    fun detectUciMoves(state1: Map<String, Set<String>>?, state2: Map<String, Set<String>>?): List<String> {
        if (state1 == null || state2 == null) {
            Log.e("ChessDetector", "Ã¢ÂÅ’ One or both board states are null")
            return emptyList()
        }
        ...
        if (moves.isNotEmpty()) {
            Log.d("ChessDetector", "Ã°Å¸Å½Â¯ DETECTED MOVES:")
            moves.forEach { Log.d("ChessDetector", "   $it") }
        } else {
            Log.d("ChessDetector", "Ã¢ÂÅ’ No clear moves detected")
    ...
    fun compareBoardsAndDetectMoves(board1Path: String, board2Path: String): List<String> {
        Log.d("ChessDetector", "Ã¢â„¢Å¸Ã¯Â¸Â CHESS MOVE DETECTOR Ã¢â„¢Å¸Ã¯Â¸Â")
        ...
        return if (state1 != null && state2 != null) {
            ...
            Log.e("ChessDetector", "Ã¢ÂÅ’ Failed to process one or both boards")
            emptyList()
        }
    }
}        val gray = Mat()
