...
object ChessMoveDetector {
    ...
    fun getBoardState(imgPath: String, boardName: String): Map<String, Set<String>>? {
        ...
        val imgOrig = Imgcodecs.imread(imgPath)
        if (imgOrig.empty()) {
        ...
        val img = Mat()
        val targetWidth = 900.0
        val scale = targetWidth / imgOrig.width()
        val targetHeight = (imgOrig.height() * scale).toInt()
        Imgproc.resize(imgOrig, img, Size(targetWidth, targetHeight.toDouble()))
        imgOrig.release()
